---
title: "Add project hosts entry on Windows"
---
...
On Windows, custom DNS entries can be added to the
`C:\Windows\System32\drivers\etc` and will take precedence over the same
entries provided by any DNS server.
...
When using Docker for Windows you can use `127.0.0.1` for the IP
address.

1.  Open `C:\Windows\System32\drivers\etc` with admistrative privileges
    via `notepad.exe` or any other text editor.
